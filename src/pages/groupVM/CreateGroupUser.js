...
import { Form, Button, Modal, Input, Spin, Tree, message,  } from "antd";
  ...
  useEffect(() => {
    if (props.visible === true) {
      const getListVm = async () => {
        ...
        const result = await axiosClient.get(API.LIST_VM);
        if (result) {
          let vmList = [];
  ...
  return (
    ...
    >
      <Form
        ...
        onFinish={handleOk}
        initialValues={{
          name: props?.groupbackupUser ? props?.groupbackupUser?.name : undefined,
          // description: props?.groupbackupUser ? props?.groupbackupUser?.description : undefined,
          vms: props?.groupbackupUser ? props?.groupbackupUser?.vms : undefined,
        }}
      >
        <Form.Item
          name="name"
