import { Form, Button, Select, Switch, InputNumber, Input, Modal, message , TimePicker} from 'antd';
...
//   const [listUsername, setListUsername] = useState([]);
  ...
  const handleOnchangeRetentionDay= e =>{
    ...
    if (i > 1) {
      let before_next_cycle = 7 - Math.abs(day_map.get(daysOfWeek[0]) - day_map.get(daysOfWeek[i - 1]));
      for (let k = i - 1; k > 0; k--) {
  ...
  // console.log(stateInReduecer);
  // const callbackGetData = (page) => {
  ...
  useEffect(() => {
    if (props.visible === true) {
      ...
      const getListGroupid = async () => {
        const result = await axiosClient.get(API.LIST_GROUP_BACKUP);
        if (result) {
          const a = result?.data
            setListGroupbackup(a);
          } else {
          setListGroupbackup([]);
        }       
  ...
  const onChangeUser = async (e) => {
    form.setFieldsValue({ user_name: e });
  }
    ...
    <Modal title="Create policy"
      ...
      <Form {...layout} form={form} name="control-hooks" onFinish={handleOk}>
        ...
        <div className="switch-btn">
          <p>Using <a className="green-btn" href="https://docs.fptvds.vn/s/c4hh313uh0lc9ct5ascg/service-documents/d/c4j1u3juh0lc9ct5asj0/gioi-thieu-dich-vu-backup-tren-fpt-vds?currentPageId=c4j4rljuh0lc9ct5asq0" target="_blank">default policy:</a></p>
        ...
        {!showvolume ? (
            <div>
            <Form.Item label="Days of week:" name="days_of_week" 
            ...
            <Form.Item name="start_time" label="Start Time" rules={[{ required: true, message: 'Please input Start time!' }]}>
              <TimePicker  initialValues={moment('12:00', timeFormat)} format={timeFormat} onChange={e => setTimeState(e)} />      
            </Form.Item>
            <Form.Item name="retention" label="Retention" rules={[{ required: true, message: 'Please input Retention!' }]}>
              <InputNumber placeholder="Please input Retention"  max={60} min={1} />         
            </Form.Item>
            </div>
        ...
        <Form.Item {...tailLayout}>
          <Button className="btn-next" loading={loading} disabled={retentionWarning} type="primary" htmlType="submit">
